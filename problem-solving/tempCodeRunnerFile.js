...
function sockMerchant(ar, n) {
  if (n < 100) {
    ...
    const itemsToString = [];
    onlyPairs.forEach(item =>{
      itemsToString.push(item.join(' '));
    })
    const uniqueArray = itemsToString.filter(function(item, pos, self) {
      return self.indexOf(item) == pos;
    })
    const pairsArray = [];
    uniqueArray.forEach(pair => {
      pairsArray.push(pair.split(' '));
    })
    const pairsCounter = [];
    pairsArray.forEach(e => {
      pairsCounter.push(e.length / 2);
    })
    if (pairsCounter.length === 0) {
      return 0;
    }
...
console.log(sockMerchant(arr1, arr1.length));
